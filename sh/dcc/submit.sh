...
srun python src/train.py   \
    ...
    'hydra.run.dir=${paths.log_dir}/${task_name}/runs/fastflow_Ace-A-Nme-dist_loss_kl' \
    +logger.wandb.name=fastflow_Ace-A-Nme-dist_loss_kl \
    logger.wandb.project=fastflow   \
    model.lambda_kl=0.01 \
    model.kl_start_frac=0 model.align_until_frac=0.0 model.lambda_align=0 \
    model.noise_sigma=0.0   \
    trainer='ddp' trainer.strategy="ddp_find_unused_parameters_true" \
    trainer.max_epochs=3000 \
    'trainer.plugins=[]' \
    +model.student_ckpt_path=scratch/transferable-samplers/logs/train/runs/fastflow_Ace-A-Nme-kl/checkpoints/last-v2_weights_only.ckpt
